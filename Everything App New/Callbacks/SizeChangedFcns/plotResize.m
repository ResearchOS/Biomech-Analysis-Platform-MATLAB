...
% 2. Add plot button
objResize(tab.addPlotButton, [0.18 0.95], [0.05 compHeight]);

% 3. Remove plot button
objResize(tab.removePlotButton, [0.24 0.95], [0.05 compHeight]);

% 4. Sort plots drop down
objResize(tab.sortPlotsDropDown, [0.2 0.9], [0.1 compHeight]);

% 5. All plots UI tree
objResize(tab.allPlotsUITree, [0.01 0.5], [0.3 0.4]);

% 6. Plots search field
objResize(tab.plotSearchField, [0.01 0.9], [0.15 compHeight]);

% 7. Component label
objResize(tab.componentLabel, [0.01 0.45], [0.1 compHeight]);

% 8. Add component button
objResize(tab.addComponentButton, [0.18 0.45], [0.05 compHeight]);

% 9. Remove component button
objResize(tab.removeComponentButton, [0.24 0.45], [0.05 compHeight]);

% 10. Sort components drop down
objResize(tab.sortComponentsDropDown, [0.2 0.4], [0.1 compHeight]);

% 11. All components UI tree
objResize(tab.allComponentsUITree, [0.01 0.01], [0.3 0.39]);

% 12. Components search field
objResize(tab.componentSearchField, [0.01 0.4], [0.15 compHeight]);
...
% PlotLevelDropDownRelPos=[0.22 0.3];
