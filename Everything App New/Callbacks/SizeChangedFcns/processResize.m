...
% 1. Variables label
objResize(tab.variablesLabel, [0.01 0.95], [0.1 compHeight]);

% 2. Add variable button
objResize(tab.addVariableButton, [0.18 0.95], [0.05 compHeight]);

% 3. Remove variable button
objResize(tab.removeVariableButton, [0.24 0.95], [0.05 compHeight]);

% 4. Sort variables drop down
objResize(tab.sortVariablesDropDown, [0.2 0.9], [0.1 compHeight]);

% 5. All variables UI tree
objResize(tab.allVariablesUITree, [0.01 0.5], [0.3 0.4]);

% 6. Variables search field
objResize(tab.variablesSearchField, [0.01 0.9], [0.15 compHeight]);

% 7. Functions label
objResize(tab.processLabel, [0.01 0.45], [0.1 compHeight]);

% 8. Add function button
objResize(tab.addProcessButton, [0.18 0.45], [0.05 compHeight]);

% 9. Remove function button
objResize(tab.removeProcessButton, [0.24 0.45], [0.05 compHeight]);

% 10. Sort functions drop down
objResize(tab.sortProcessDropDown, [0.2 0.4], [0.1 compHeight]);

% 11, All functions UI tree
objResize(tab.allProcessUITree, [0.01 0.01], [0.3 0.39]);

% 12. Functions search field
objResize(tab.processSearchField, [0.01 0.4], [0.15 compHeight]);
