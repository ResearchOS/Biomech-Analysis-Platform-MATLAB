...
% varOutRowIdxNums = find(contains(links(:,1),abbrevs1)==1); % Links is N x 3: [PR, VR, PR]

% for i=1:length(varOutRowIdxNums)
    % varUUID = links(varOutRowIdxNums(i),2); % The UUID of the output variable.

varOutRowIdx = contains(links(:,1),abbrevs1) & contains(links(:,2),abbrevs2);
varInRowIdx = contains(links(:,3),abbrevs1) & contains(links(:,2),abbrevs2); % The row numbers where the output variable is an input variable.
...
s = [s; links(varOutRowIdx,1)]; % The UUID of the PR that the variable is an output of.
t = [t; links(varInRowIdx,3)]; % The UUID of the PR that the variable is an input to.

edgeNames = [edgeNames; links(varInRowIdx,2)];
