...
if ~ismember(headerName,logsheetVar(1,:))
    disp(['Subject Codename Column Header name not found in first row of logsheet: ' headerName]);
    return;
end

if all(ismember({targetTrialIDColHeader,headerName},logsheetVar(1,:)))
    subjCodenames=logsheetVar(numHeaderRows+1:end,ismember(logsheetVar(1,:),headerName));
    ...
    logsheetVar(numHeaderRows+1:end,ismember(logsheetVar(1,:),headerName))=subjCodenames;
