...
Q=uifigure('Name','Edit Pub Table','DeleteFcn',@(Q,event) pubTableEditWindowDeleteFcn(Q,fig));
...
% Qhandles.cellLabel=uilabel(Q,'Text','Cell','Position',[10 400 100 30]);
% Qhandles.cellDropDown=uidropdown(Q,'Items',{''},'Position',[60 400 100 30],'ValueChangedFcn',@(cellDropDown,event) cellDropDownValueChanged(cellDropDown));
% Qhandles.literalVarDropDown=uidropdown(Q,'Items',{'Literal','Variable'},'Position',[200 400 100 30]);
% Qhandles.summDropDown=uidropdown(Q,'Items',{'mean ± std.','median ± IQR'},'Position',[300 400 100 30]);
Qhandles.tablePanel=uipanel(Q,'BackgroundColor',[0.9 0.9 0.9],'Position',[10 10 580 430],'Scrollable','on');
